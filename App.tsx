...
export default function App() {
    ...
    const handleSendMessage = async (text: string, isSelection: boolean = false) => {
        ...
        if (isProgression) {
            if(!text.startsWith("Looks great") && !text.includes("let's draft")) newBookState = { ...bookState, [currentStepName]: text };
            const updatedProgress = getUpdatedProgress(progress, currentStepName);
            setProgress(updatedProgress);
            nextStepIndex = currentStepIndex + 1;
        }
        if (nextStepIndex >= flatSteps.length) return;
    ...
    const handleOutlineResponse = (response: any) => {
     ...
     const handleApproveOutline = () => {
        ...
        setTimeout(() => {
            const nextTask = newFlatSteps[firstChapterStepIndex];
            handleSendMessage(`Draft ${nextTask.step}`, false);
        }, 100);
    };

    const handleChapterDraftResponse = (response: any, chapterIndexInFlatSteps: number) => {
    ...
    const exportBook = (type: ExportType) => { /* Placeholder */ };
            ...
            <div className="flex flex-row flex-1 overflow-hidden">
                ...
                <main className="flex-1 flex flex-col min-w-0 h-full bg-gray-850">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        ...
                        {mainView === 'editor' && <MarkdownEditor bookState={bookState} onContentChange={handleContentChange} exportBook={exportBook} />}
